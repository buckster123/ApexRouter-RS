...
| **D10** | **Money safety is structural.** The ledger row is written **before** the billing call; `ledger.jsonl` is append-only and "active" is a *query*; a `SpendApproval` cannot be fabricated by any code path; there is a daemon-side hard ceiling. **Nothing that costs money is auto-destroyed on shutdown, crash, or daemon restart, at any setting.** *Carve-out (2026-08-07):* the **boot watchdog** for an in-flight rental may destroy a box that never reaches healthy within `max_boot_secs` (or that stalls on a fatal `status_msg`) — that is stopping the meter on a wedged bill, not shutdown cleanup. See amendments log. | The failure it prevents has already happened in this codebase. A crash must not delete a paid box, and a leak must be visible as an alert rather than as a surprise on the invoice. A wedged boot that bills forever is the same class of leak in reverse. |
...
- **2026-08-07** — **D10 letter clarified: boot-watchdog auto-destroy is the one carve-out.**
  The absolute "never auto-destroyed, at any setting" wording conflicted with the intentional
  boot watchdog (`providers/src/vast/boot.rs`), which destroys a rented instance that never
  reaches healthy within `max_boot_secs` or that stalls on a repeated fatal `status_msg`. That
  path exists so a wedged box does not bill overnight; shutdown, crash, `down`, and
  `kill_children_on_exit` still **never** destroy a paid instance. D10's table row now says so.
  Also from the full-repo audit the same day: package (a) mounted the proxy mutation gate and
